...
| `AIF_RUN_BROKER_PORT`                                  | number  | `3013`                         | Port the "Run" feature's process broker binds inside the agent container. Always on (no feature flag) — see [Run feature](#run-feature)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
Lets a task's project actually be executed (not just planned/implemented/reviewed) from the Review or Done tab, with live stdout/stderr and a Stop button — see [Architecture → Run pipeline](architecture.md#run-pipeline) for the full design and the `.ai-factory/HOW-TO-RUN.md` contract, and [API Reference](api.md#run) for the endpoints and WebSocket events.

- **Always on, no LLM per click.** Clicking Run reads `.ai-factory/HOW-TO-RUN.md` from the task's worktree (or project root) and spawns exactly that command — deterministic, no runtime cost per run. An LLM ("run-inspector") only runs once to bootstrap the file if it's missing, or on-demand via "Re-inspect".
- **`implement-coordinator` keeps the file current** as part of its normal job — no separate maintenance step.
- **Reachability depends on how AIF itself is deployed.** Bare-metal: the spawned process is reachable exactly like running it yourself, no configuration needed. Dockerized: the `agent` container is network-isolated by default, so a raw process's port is not reachable from the browser unless you opt into Docker-socket execution (below).
- **Docker-socket execution (opt-in, two gates required together):** `AIF_AGENT_DOCKER_SOCKET_ENABLED=true` on the agent (this doc's [Environment Variables](#environment-variables) table) **and** the project's own "Allow Docker-based Run" toggle in Project Runtime Settings. With both set and the socket mounted (`docker-compose.yml`, commented out by default), `docker`/`docker compose`-type commands talk to the host's real Docker daemon (normal port publishing, works the same on Windows/Mac/Linux Docker Desktop), and raw `process`-type commands get wrapped in `docker run -p <port>:<port>` using `AIF_RUN_WRAPPER_IMAGE`.
- **Security:** the Docker socket is equivalent to root on the host. Combined with the agent's existing LLM-driven shell access, enabling this raises the blast radius of a prompt-injection or runaway-agent scenario from "contained to the worktree" to "full host compromise". Enable per-project only where you've accepted that trade-off — never as a blanket default.
- **One active run per project** — starting a second run while one is active returns `409`.
- **Idle timeout** (30 min with no output) and **max-runtime timeout** (6h hard ceiling) both auto-stop a forgotten run; the Stop button sends `SIGTERM` (clean shutdown, including `docker compose down`-equivalent behavior for foreground `docker compose up`).
...
- [Architecture](architecture.md) — how the agent pipeline uses these settings, including the [Run pipeline](architecture.md#run-pipeline)
- [API Reference](api.md#run) — Run feature endpoints and WebSocket events
